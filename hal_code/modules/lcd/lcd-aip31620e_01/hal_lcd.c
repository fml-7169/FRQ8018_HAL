...
void lcd_set_seg(unsigned char seg_num)
{
	unsigned char addr;
	addr = 12- seg_num/8;
	__lcd_ram[addr] |=(0x01<<((seg_num%8)%8));
}
...
void lcd_tile(int row,int type,int enable){
    ...
    if(type == LCD_TYPE_BLE){
	 ...
	 if(enable){
	 	 lcd_clear_seg(HT_T2_LCD);
		 lcd_set_seg(HT_T1_LCD);
	 }
	 else{
	 	 lcd_clear_seg(HT_T1_LCD);
		 lcd_set_seg(HT_T2_LCD);
		 
	 }
	 }
	else if(type == LCD_TYPE_DEFAULT){
	 if(enable)
		 lcd_set_seg(HT_T_LCD);	
	 else
		 lcd_clear_seg(HT_T_LCD);
	 }
	sendDataToLcdDriver();
