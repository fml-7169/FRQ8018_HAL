...
#define NEGATIVE 0b01000000 //'-'
 ...
 static const unsigned char lcd_num_arr[CHAR_POS_LEN][CHAR_SEG_LEN] = {
     ...
     {HT_13A_LCD,HT_13B_LCD,HT_13C_LCD,HT_13D_LCD,HT_13E_LCD,HT_13F_LCD,HT_13G_LCD},
 };
...
void lcd_set_seg(unsigned char seg_num)
{
	unsigned char addr;
	addr =seg_num/8;
	__lcd_ram[addr] |=(0x01<<((7-(seg_num%8))%8));
}
...
{    
	...
	unsigned char i = 0;
	unsigned char electry_icon[BATT_LEVEL]={HT_T1_LCD,HT_T2_LCD,HT_T3_LCD,HT_T4_LCD,HT_T5_LCD};    
	if(electry<=10){
		electry_level = 0;
	}
	else if(electry<=25){
...
void lcd_tile(int row,int type,int enable){
    ...
    if(type == LCD_TYPE_BLE){
        if(enable)
            lcd_set_seg(HT_L1_LCD);    
        else
            lcd_clear_seg(HT_L1_LCD);
    }else if(type == LCD_TYPE_HUNDRED){
	 if(enable)
		 lcd_set_seg(HT_L9_LCD);	
	 else
		 lcd_clear_seg(HT_L9_LCD);
	}else if(type == LCD_TYPE_NEGATIVE){
		 if(enable)
			 lcd_set_seg(HT_L8_LCD);	
		 else
			 lcd_clear_seg(HT_L8_LCD);
	 }else if(type == LCD_TYPE_UNIT){
		 if(enable){
		 	 lcd_clear_seg(HT_L12_LCD);
			 lcd_set_seg(HT_L13_LCD);
		 }
		 else{
		 	 lcd_clear_seg(HT_L13_LCD);
			 lcd_set_seg(HT_L12_LCD);	 
	 	}
	 }else if(type == LCD_TYPE_DEFAULT){
	 if(enable){
		 lcd_set_seg(HT_L2_LCD);
		 
		 lcd_set_seg(HT_L0_LCD);	
	 
	 	 lcd_set_seg(HT_L10_LCD);
	 	 lcd_set_seg(HT_L11_LCD);	
	 	 lcd_set_seg(HT_L17_LCD);	
	 	 lcd_set_seg(HT_L16_LCD);	 
	 	 lcd_set_seg(HT_L15_LCD);	
	 	 lcd_set_seg(HT_L12_LCD);
#if 0
		// lcd_set_seg(HT_L6_LCD);	
		// lcd_set_seg(HT_L5_LCD);	
		 lcd_putchar_cached(0,'1'); 
		 lcd_putchar_cached(1,'2'); 
		 lcd_putchar_cached(2,'3'); 
