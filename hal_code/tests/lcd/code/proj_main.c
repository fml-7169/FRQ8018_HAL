...
__attribute__((section("ram_code"))) void user_entry_after_sleep_imp(void)
{
 ...
 void Init_ps_hold_pin()
{
	
 	co_printf("Init_ps_hold_pin\r\n");
	pmu_set_port_mux(GPIO_PORT_C,GPIO_BIT_1,PMU_PORT_MUX_GPIO);
	pmu_set_pin_to_PMU(GPIO_PORT_C,BIT(1));   
	pmu_set_pin_dir(GPIO_PORT_C,BIT(1), GPIO_DIR_OUT);
	system_set_port_pull(GPIO_PC1,true);
	pmu_set_gpio_value(GPIO_PORT_C,BIT(1), 1);
