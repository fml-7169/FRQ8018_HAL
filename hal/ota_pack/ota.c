...
void __attribute__((weak)) ota_stop(ota_warning_type evt_id)
{
    ...
    ota_deinit(0);
    ota_init(0);//²»¶ÏÏßµÄÇé¿öÏÂ£¬ÖØÐÂ³õÊ¼»¯
    g_otas_get_status = 0;
...
void app_otas_recv_data(uint8_t conidx,uint8_t *p_data,uint16_t len)
{
    ...
    // Ö§³ÖÊÖ»ú¶Ë½«°ü´ÓÓ¦ÓÃ²ã½øÐÐ²ð·ÖµÄ¹¦ÄÜ£¬¶ø²»ÊÇÓ¦ÓÃ²ã·¢³¤°ü£¬L2CAPÈ¥²ð·Ö¡£
    if(ota_recving_data) {
        // check data idx
    ...
    switch(cmd_hdr->opcode)
    {
        ...
        case OTA_CMD_READ_DATA:
            ...
            {
                // Êý¾ÝÌ«³¤£¬²»ÄÜÍ¨¹ýnotify·µ»Ø£¬Í¨Öªclient²ÉÓÃread·½Ê½»ñÈ¡
...
//write user data.
        ...
        case OTA_CMD_REBOOT:
            if(first_pkt.buf != NULL)
            {
               ...
               if(app_otas_crc_cal(ota_packet_total_len,new_bin_base,bin_crc_data)){
               rsp_hdr->result = OTA_RSP_SUCCESS;
#ifdef FLASH_PROTECT
                flash_protect_disable(0);
#endif	
