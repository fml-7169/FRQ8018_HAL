...
#define FR_IMAGE_INFO_LEN   256
#define FR_IMAGE_CRC_POS    0
#define RF_IAMGE_CRC_LEN    4
...
int main (int argc, char *argv[])
{
    ...
    int fileLen = ftell(fp);                                // 得到文件长度

    if(fileLen<FR_IMAGE_INFO_LEN){
        printf("source file len to short\n");
        return -1;
    }

    int image_date_len = fileLen - FR_IMAGE_INFO_LEN;

    fseek(fp, FR_IMAGE_INFO_LEN, SEEK_SET);                // 定位到数据头

    //1.3 read file and check crc
    char buff[BUFFSIZE];
    int count = 0;
    bzero (buff, BUFFSIZE);
    while (!feof (fp)){
        count = fread (buff, sizeof (char), BUFFSIZE, fp);
        int n = feof (fp);
        //printf ("%d,%d\n", count, n);
        file_crc_value = Crc32CalByByte(file_crc_value,buff,count);
    }
    printf("get file crc %08X\r\n",file_crc_value);
    // 2 .write crc to image
    fseek(fp, FR_IMAGE_CRC_POS, SEEK_SET);                // 定位到数据头
    uint8_t crc_buf[4] = {0x00};

#if 0
    crc_buf[0] = (uint8_t )((file_crc_value>>24)&0xff);
    crc_buf[1] = (uint8_t )((file_crc_value>>16)&0xff);
    crc_buf[2] = (uint8_t )((file_crc_value>>8)&0xff);
    crc_buf[3] = (uint8_t )((file_crc_value)&0xff);

    printf("%02x,%02x,%02x,%02x\r\n",crc_buf[0],crc_buf[1],crc_buf[2],crc_buf[3]);
#endif
    nbytes  = fwrite( (void *)( &file_crc_value), 1, sizeof(file_crc_value), fp);
    if (nbytes != sizeof(file_crc_value) )
    {
